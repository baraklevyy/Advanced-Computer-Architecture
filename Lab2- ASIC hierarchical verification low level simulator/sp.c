...
#define SP_SRAM_HEIGHT	64 * 1024
	...
	int DMA[6]; //0 - src_address, 1 - des_adress, 2 - len, 3- state, 4 - remain, 5 - flag
...
void* DMA_thread(void* args){
    int i;
    sp_t* sp = (sp_t*) args;
    sp->DMA[4] = sp->DMA[2];
    for(i = 0; i < sp->DMA[2]; i++){
        sp->sram->data[sp->DMA[1] + i] = sp->sram->data[sp->DMA[0] + i];
        sp->DMA[4]-=1;
    }
	if(sp->DMA[4] == 0) sp->DMA[3] = DMA_WAIT;
    return (void *) 0;
}
...
static void sp_ctl(sp_t *sp)
{
	...
	case CTL_STATE_DEC0:
        ...
        if (0 != (spro->inst & 32768)) sprn->immediate += (65535 << 16);
	...
	case CTL_STATE_DEC1:
		...
		if(spro->opcode != LHI){
			...
			if(spro->src0 == 0) sprn->alu0 = 0;
			else if(spro->src0 == 1) sprn->alu0 = spro->immediate;

			sprn->alu1=spro->r[spro->src1];
			if(spro->src1 == 0) sprn->alu1 = 0;
			else if(spro->src1 == 1) sprn->alu1 = spro->immediate;
		}
		else{
	...
	case CTL_STATE_EXEC0:
		...
		if (spro->opcode == ADD) sprn->aluout = spro->alu0 + spro->alu1;
		else if (spro->opcode == SUB) sprn->aluout = spro->alu0 - spro->alu1;
		else if (spro->opcode == LSF) sprn->aluout = spro->alu0 << spro->alu1;
		else if (spro->opcode == RSF) sprn->aluout = spro->alu0 >> spro->alu1;
		else if (spro->opcode == AND) sprn->aluout = spro->alu0 & spro->alu1;
		else if (spro->opcode == OR) sprn->aluout = spro->alu0 | spro->alu1;
		else if (spro->opcode == XOR) sprn->aluout = spro->alu0 ^ spro->alu1;
		else if (spro->opcode == LHI) sprn->aluout = (spro->alu0 & 65535) + (spro->alu1 << 16);
		else if (spro->opcode == LD) llsim_mem_read(sp->sram, spro->alu1);
		else if (spro->opcode == ST || spro->opcode == HLT) sprn->aluout = 0;
		else if (spro->opcode == JLT){
			if (spro->alu0 < spro->alu1) sprn->aluout = 1;
			else sprn->aluout = 0;
		}
		...
		else if (spro->opcode == JIN) sprn->aluout = 1;
		else if(spro->opcode == COPY){
			if(sp->DMA[3] == DMA_IDLE){
				sp->DMA[3] = DMA_WAIT;
				sp->DMA[5] = 1;
			}
		}
	...
	case CTL_STATE_EXEC1:
		...
		else sprn->ctl_state = CTL_STATE_FETCH0;
		...
		if(spro->opcode == LD){
	        ...
	        while(DMA_WAIT == !sp->DMA[3]){}
	        sp->DMA[0] = spro->r[spro->src0];
	        sp->DMA[1] = spro->r[spro->dst];
	        sp->DMA[2] = spro->r[spro->src1];
	        sp->DMA[3] = DMA_START;
	        if(pthread_create(&sp->DMA_thread, NULL, DMA_thread,(void*) sp)) fprintf(stderr, "error launching DMA thread\n");
	        sprn->pc = spro->pc + 1;
		}
		else if(spro->opcode == POLL){
	        fprintf(inst_trace_fp, ">>>> EXEC: POLL - Remaining copy: %i <<<<\n\n", sp->DMA[4]);
	        sprn->r[spro->dst] = sp->DMA[4];
	        sprn->pc = spro->pc + 1;
		}
		else{
